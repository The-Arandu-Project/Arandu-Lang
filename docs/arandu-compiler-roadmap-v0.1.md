...
| 2026-09 | Codex | **Ownership sensível a campos na rc.5**: `Loan` passou a preservar o caminho compacto de projeções do `AmirPlace`; empréstimos de campos com `SymbolId` distintos deixam de produzir O003 falso ao acessar campos irmãos, enquanto raízes, prefixes, índices e dereferences permanecem conservadores. Uma matriz tipada compartilhado/exclusivo também prova classificação idêntica em CFG linear e diamond. |
