...
use arandu_query::{
    DirectoryListing, LocalPackageGraph, MANIFEST_FILENAME, ManifestDependency, ManifestError,
    ManifestSpelling, MaterializedGitPackage, ModuleRoots, ProjectManifest, STDLIB_ENV,
    StdlibResolveOpts, StdlibRoot, ensure_toolchain_compatible, find_manifest, load_manifest,
...
pub fn load_project(
    ...
    let remote_packages = materialize_remote_packages(&root, &manifest_path, &data, &cache, flags)?;
    let package_graph =
...
fn materialize_remote_packages(
    root: &Path,
    manifest_path: &Path,
    ...
    let locked_digests = read_locked_git_digests(root, flags.locked)?;
    let store = crate::cache::CacheStore::new(cache.clone());
    ...
    while let Some((manifest, data, boundary)) = queue.pop_front() {
        ...
        if packages > arandu_query::PackageGraphLimits::default().max_packages {
        ...
        for (alias, dependency) in &data.dependencies {
            match dependency {
                ...
                ManifestDependency::Git { origin, commit } => {
                    ...
                    if flags.locked && expected.is_none() {
                        return Err(format!(
...
fn read_locked_git_digests(
    root: &Path,
    strict: bool,
) -> Result<BTreeMap<String, arandu_query::CacheDigest>, String> {
    ...
    let lock = match arandu_query::Lockfile::parse(&path, &text) {
        Ok(lock) => lock,
    ...
    lock.packages
        ...
        .filter_map(|package| {
            package.content_digest.map(|digest| {
                digest
                    .parse::<arandu_query::CacheDigest>()
